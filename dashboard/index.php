...
$router_id = $stmt->fetchColumn();

/* ======================
   METRICS
====================== */

$stmt = $pdo->prepare("SELECT COUNT(*) FROM hotspot_users WHERE status='active' AND router_id=?");
$stmt->execute([$router_id]);
$activeUsers = $stmt->fetchColumn();

$stmt = $pdo->prepare("SELECT COUNT(*) FROM hotspot_users WHERE status='expired' AND router_id=?");
$stmt->execute([$router_id]);
$expiredUsers = $stmt->fetchColumn();

$stmt = $pdo->prepare("SELECT COUNT(*) FROM hotspot_users WHERE router_id=?");
$stmt->execute([$router_id]);
$totalUsers = $stmt->fetchColumn();

$stmt = $pdo->prepare("SELECT COUNT(*) FROM hotspot_profiles WHERE router_id=?");
$stmt->execute([$router_id]);
$totalPackages = $stmt->fetchColumn();

$stmt = $pdo->prepare("SELECT COUNT(*) FROM routers WHERE router_id=? AND online_status='online'");
$stmt->execute([$router_id]);
$routersOnline = $stmt->fetchColumn();

$stmt = $pdo->prepare("SELECT COUNT(*) FROM routers WHERE router_id=?");
$stmt->execute([$router_id]);
$totalRouters = $stmt->fetchColumn();
...
$stmt = $pdo->prepare("
    ...
    WHERE status='pending' AND router_id=?
");
$stmt->execute([$router_id]);
...
$stmt = $pdo->prepare("
    ...
    WHERE p.status='used' AND p.router_id=?
...
$stmt = $pdo->prepare("
    ...
    LEFT JOIN hotspot_users hu 
        ON hu.plan_id = hp.id AND hu.router_id=?
    WHERE hp.router_id=?
    GROUP BY hp.id
...
$stmt = $pdo->prepare("
    ...
    WHERE router_id=?
    AND status='active'
    AND expires_at BETWEEN CURDATE() 
                ...
                <div class="container-xl px-1 mt-4">
                    ...
                    <?php if (empty($router_id)): ?>
                        ...
                        <div class="alert alert-warning text-center mt-4">
                            <h5 class="mb-2">
                                <i class="fa fa-router me-2"></i>No Router Connected
                            </h5>
                            <p class="mb-3">You need to add your first router to start managing hotspot users.</p>

                            <a href="../routers" class="btn btn-primary">
                                <i class="fa fa-plus me-2"></i>Add Your First Router
                            </a>
                        </div>
                                ...
                                <div class="card card-raised border-start border-success border-4 h-100">
                                    <div class="card-body">
                                        <div class="d-flex align-items-center">
                                            <?php $activePercent = $totalUsers > 0
                                ...
                                <div class="card card-raised shadow-sm mb-4">
                                    <div class="card-header bg-primary text-white">
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <a href="../plans/payments" class="btn btn-sm btn-light">View All</a>
                                        </div>
                                    </div>
                            ...
                            <div class="col-lg-4">
                                ...
                                <div class="card card-raised shadow-sm mb-4">
                                    ...
                                    <div class="card-body d-grid gap-2">
                                        <a href="../hotspotUsers/index" class="btn btn-success">
                                            <i class="fa fa-user-plus me-2"></i>Add New User
                                        </a>
                                        <a href="../hotspotProfiles/profiles" class="btn btn-primary">
                                            <i class="fa fa-box me-2"></i>Create Package
                                        </a>
                                        <a href="../plans/payments" class="btn btn-outline-primary">
                                        ...
                                        <a href="../plans/invoices" class="btn btn-outline-info">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Chart(revenueCtx, {
                ...
                data: {
                    labels: [
                        <?php foreach ($revenueByDay as $r): ?> '<?= date('M d', strtotime($r['date'])) ?>',
                        <?php endforeach; ?>
                    ],
                    datasets: [{
                        label: 'Revenue (KES)',
            ...
            new Chart(userTypeCtx, {
                ...
                data: {
                    labels: [
                        <?php foreach ($usersByType as $ut): ?> '<?= strtoupper($ut['user_type']) ?>',
                        <?php endforeach; ?>
                    ],
                    datasets: [{
                        data: [
