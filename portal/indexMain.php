...
if (!empty($user)) {
    ...
    if ($limit && $used !== $limit) {
        ...
        <style>body{background:#0a0a14;display:flex;align-items:center;justify-content:center;min-height:100vh;font-family:sans-serif;color:#fff;margin:0;}p{opacity:.6;font-size:14px;}</style>
        </head>
        <body>
...
.support-bar svg { width: 13px; height: 13px; fill: var(--accent); flex-shrink: 0; }
.support-bar a   { color: var(--accent); font-weight: 600; text-decoration: none; }
...
/* ── HEADER ── */
.header { text-align: center; padding: 44px 0 32px; animation: fadeDown 0.6s ease both; }
.logo-ring {
...
.logo-ring svg { width: 26px; height: 26px; fill: var(--accent); }
.header h2 {
  font-family: 'Playfair Display', serif;
  font-size: 28px; font-weight: 700; color: var(--text); letter-spacing: -0.3px;
  margin: 0;
}
.subtitle { margin-top: 6px; color: var(--muted); font-size: 14px; font-weight: 300; }
...
.plan:hover { border-color: rgba(42,125,79,0.3); transform: translateY(-2px); box-shadow: var(--shadow-md); }
.plan:hover::before { opacity: 1; }
.plan:active { transform: translateY(0); }
.plan:nth-child(1) { animation-delay: 0.05s; }
.plan:nth-child(2) { animation-delay: 0.12s; }
.plan:nth-child(3) { animation-delay: 0.19s; }
.plan:nth-child(4) { animation-delay: 0.26s; }
.plan:nth-child(5) { animation-delay: 0.33s; }

.plan-row  { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.plan-left { display: flex; align-items: center; gap: 13px; }
.plan-dot  {
...
.plan-badges { display: flex; gap: 6px; margin-top: 5px; flex-wrap: wrap; }
.plan-badge  {
...
.price {
  ...
  white-space: nowrap; text-align: right; line-height: 1; margin: 0;
}
.price-kes {
  font-family: 'Outfit', sans-serif; font-size: 11px;
  font-weight: 400; color: var(--muted); display: block; margin-top: 2px;
}
.validity { display: none; }
.plan > button { display: none; }

.plan-arrow { color: var(--muted); font-size: 20px; transition: transform 0.2s, color 0.2s; }
.plan:hover .plan-arrow { transform: translateX(3px); color: var(--accent); }
...
.site-footer {
  ...
  width: 100%; max-width: 440px;
  margin-top: 48px;
  padding: 28px 0 env(safe-area-inset-bottom, 28px);
  animation: fadeUp 0.7s ease 0.4s both;
}

/* top ruled line with fade-out on edges */
.site-footer::before {
  content: '';
  display: block;
  height: 1px;
  background: linear-gradient(to right, transparent, var(--border-md) 30%, var(--border-md) 70%, transparent);
  margin-bottom: 24px;
}
...
.footer-brand-icon svg { width: 14px; height: 14px; fill: var(--accent); }
.footer-brand-name {
  font-family: 'Playfair Display', serif;
...
.footer-contact svg { width: 13px; height: 13px; fill: var(--accent); flex-shrink: 0; }
.footer-contact span { font-size: 12px; color: var(--text2); font-weight: 600; }

/* Copyright */
.footer-copy {
  font-size: 11px; color: var(--muted); font-weight: 300;
  letter-spacing: 0.2px; text-align: center; line-height: 1.6;
}
.footer-copy strong { font-weight: 500; color: var(--muted); }
...
.modal-handle {
  ...
  background: var(--warm2); margin: 14px auto 0;
}
...
.modal-inner { padding: 12px 24px 28px; }
...
.amount-note { font-size: 11px; color: var(--muted); margin-top: 2px; }

.form-area { padding: 16px 24px 0; }
...
.form-group small { font-size: 12px; color: var(--muted); margin-top: 6px; display: block; }

.phone-wrap {
  display: flex; background: var(--bg);
  border: 1.5px solid var(--border-md); border-radius: 13px; overflow: hidden;
  transition: border-color 0.2s, box-shadow 0.2s;
}
.phone-wrap:focus-within { border-color: var(--accent); box-shadow: 0 0 0 3px var(--accent-lt); }
.phone-prefix {
  padding: 14px 13px; background: var(--warm1);
  color: var(--accent); font-size: 14px; font-weight: 600;
  border-right: 1px solid var(--border-md); white-space: nowrap;
}
#phoneInput {
  flex: 1; background: transparent; border: none; outline: none;
  color: var(--text); font-family: 'Outfit', sans-serif;
  font-size: 16px; padding: 14px 13px; letter-spacing: 1px; width: 100%;
}
#phoneInput::placeholder { color: var(--muted); letter-spacing: normal; font-size: 14px; }
...
.btn-pay:hover { background: var(--accent-dk); transform: translateY(-1px); }
.btn-pay svg { width: 17px; height: 17px; fill: #fff; }

.mpesa-note {
  display: flex; align-items: center; justify-content: center;
  gap: 6px; font-size: 12px; color: var(--muted); margin-top: 12px;
  padding-bottom: 4px;
}
.mpesa-note svg { width: 13px; height: 13px; fill: var(--muted); }

/* ── Loading ── */
#loadingState .loading-container { text-align: center; padding: 36px 24px; }
#loadingState h3 {
  font-family: 'Playfair Display', serif; font-size: 18px;
  color: var(--text); margin: 18px 0 8px;
}
#loadingState .loading-text { font-size: 13px; color: var(--muted); }

.spinner {
  width: 44px; height: 44px; margin: 0 auto;
  border: 3px solid var(--warm2); border-top: 3px solid var(--accent);
  border-radius: 50%; animation: spin 0.9s linear infinite;
}

/* ── STK Waiting ── */
#stkWaiting .stk-container { text-align: center; padding: 12px 24px 24px; }

.arc-wrap { position: relative; width: 104px; height: 104px; margin: 0 auto 10px; }
.arc-svg  { position: absolute; inset: 0; width: 104px; height: 104px; overflow: visible; }
.arc-track { fill: none; stroke: var(--warm2); stroke-width: 3.5; }
.arc-fill  {
  fill: none; stroke: var(--accent); stroke-width: 3.5; stroke-linecap: round;
  stroke-dasharray: 238; stroke-dashoffset: 0;
  transform: rotate(-90deg); transform-origin: 52px 52px;
  transition: stroke-dashoffset 1s linear, stroke 0.5s ease;
}
.arc-fill.warn   { stroke: var(--warning); }
...
.arc-time  { font-family: 'Playfair Display', serif; font-size: 21px; font-weight: 700; color: var(--text); line-height: 1; transition: color 0.5s; }
.arc-label { font-size: 9px; color: var(--muted); letter-spacing: 1px; text-transform: uppercase; margin-top: 2px; }

.waiting-dots { display: flex; gap: 5px; justify-content: center; margin: 10px 0 14px; }
.waiting-dots span {
  width: 6px; height: 6px; border-radius: 50%; background: var(--warm2);
  animation: dotBeat 1.5s ease-in-out infinite; display: inline-block;
}
.waiting-dots span:nth-child(2) { animation-delay: 0.25s; }
.waiting-dots span:nth-child(3) { animation-delay: 0.5s;  }
@keyframes dotBeat { 0%,80%,100%{transform:scale(1);background:var(--warm2);} 40%{transform:scale(1.6);background:var(--accent);} }

#stkWaiting h3 { font-family: 'Playfair Display', serif; font-size: 18px; color: var(--text); margin-bottom: 6px; }
.stk-text    { font-size: 13px; color: var(--muted); margin-bottom: 4px; }
.stk-subtext { font-size: 12px; color: var(--muted); margin-top: 8px; }

.steps-list { display: flex; flex-direction: column; margin: 14px 0; text-align: left; }
...
.sdot-num   { width: 22px; height: 22px; border-radius: 50%; flex-shrink: 0; margin-top: 1px; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; }
.s-done     { background: var(--accent-lt); color: var(--accent); }
.s-active   { background: rgba(42,125,79,0.15); color: var(--accent); animation: stepRing 1.5s ease infinite; }
.s-wait     { background: var(--warm1); color: var(--muted); }
@keyframes stepRing { 0%,100%{box-shadow:0 0 0 0 rgba(42,125,79,0.4);}50%{box-shadow:0 0 0 5px rgba(42,125,79,0);} }
.step-label { font-size: 13px; color: var(--text); font-weight: 500; line-height: 1.3; }
.step-hint  { font-size: 11px; color: var(--muted); margin-top: 1px; }

.phone-animation, .pulse-ring { display: none; }

/* ── Success ── */
#successState .success-container { text-align: center; padding: 12px 24px 28px; }
.confetti { font-size: 28px; margin-bottom: 10px; animation: bounceIn 0.5s ease both; }
.success-icon {
  width: 66px; height: 66px; border-radius: 50%;
  background: var(--accent-lt); border: 2px solid rgba(42,125,79,0.35);
  margin: 0 auto 16px; display: flex; align-items: center; justify-content: center;
  animation: popIn 0.4s cubic-bezier(0.175,0.885,0.32,1.275) both;
}
.success-icon svg { width: 28px; height: 28px; fill: var(--accent); }
#successState h3 { font-family: 'Playfair Display', serif; font-size: 19px; color: var(--text); margin-bottom: 6px; }
#successState p  { font-size: 13px; color: var(--muted); }

/* ── Error ── */
#errorState .error-container { text-align: center; padding: 12px 24px 28px; }
.error-icon {
  width: 66px; height: 66px; border-radius: 50%;
  background: var(--danger-lt); border: 1.5px solid rgba(192,57,43,0.3);
  margin: 0 auto 16px; display: flex; align-items: center; justify-content: center;
  animation: popIn 0.4s cubic-bezier(0.175,0.885,0.32,1.275) both;
}
.error-icon svg { width: 26px; height: 26px; fill: var(--danger); }
#errorState h3 { font-family: 'Playfair Display', serif; font-size: 19px; color: var(--text); margin-bottom: 6px; }
#errorMessage  { font-size: 13px; color: var(--muted); margin-bottom: 18px; }
.btn-retry {
  width: 100%; background: var(--accent); color: #fff; border: none;
  border-radius: 13px; font-family: 'Outfit', sans-serif; font-size: 14px;
  font-weight: 600; padding: 13px; cursor: pointer;
  box-shadow: 0 4px 20px rgba(42,125,79,0.3); transition: all 0.2s;
}
.btn-retry:hover { background: var(--accent-dk); }
...
<div class="support-bar">
  ...
  Need help? Call <a href="tel:+254740770212">0740 770 212</a>
</div>
...
<footer class="site-footer">
  <div class="footer-inner">
    ...
    <div class="footer-brand">
      ...
      <div>
        <div class="footer-brand-name">Inovatech Hotspot</div>
        <div class="footer-brand-tag">Seamless connectivity</div>
      </div>
    </div>

    <!-- Contact pill -->
    <a href="tel:+254740770212" class="footer-contact">
      <svg viewBox="0 0 24 24"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>
      <span>0740 770 212</span>
    </a>

    <!-- Links -->
    <div class="footer-links">
      <a href="mailto:support@inovatech.com">Support</a>
      <div class="sep"></div>
    ...
    <!-- Copyright -->
    <p class="footer-copy">
      &copy; <?= date('Y') ?> <strong>Inovatech Systems</strong> &mdash; All rights reserved
    </p>
...
<script>
        ...
        let arcTimer = null, arcElapsed = 0;
        ...
        function startArc() {
            ...
            arcTimer = setInterval(() => {
                ...
                if (rem <= 0) { clearInterval(arcTimer); return; }
                const m = Math.floor(rem / 60), s = rem % 60;
                timeEl.textContent = m + ':' + String(s).padStart(2, '0');
                arc.style.strokeDashoffset = ((arcElapsed / TOTAL_SECS) * ARC_CIRC).toFixed(2);
                if (rem <= 15)      { arc.className = 'arc-fill danger'; timeEl.style.color = 'var(--danger)'; }
                else if (rem <= 30) { arc.className = 'arc-fill warn';   timeEl.style.color = 'var(--warning)'; }
                document.getElementById('tickerMsg').textContent = HINTS[Math.floor(arcElapsed / 15) % HINTS.length];
            }, 1000);
        }
        function stopArc() { clearInterval(arcTimer); }
            ...
            checkInterval = setInterval(async () => {
                try {
                    ...
                    if (status === 'ACTIVE') {
                        ...
                        setTimeout(() => { window.location.href = 'create.php'; }, 2000);
